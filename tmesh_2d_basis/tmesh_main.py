...
import read_input_module
import dialogue_module
################################################

##################################################################
#BLOCK : doubly linked list for storing line segments            
##################################################################
...
class DoublyLinkedList:
    ...
    # this function is for diplaying the nodes of a list - for debugging    
...
###############################################################################
#BLOCK : here we update a tree of line segments
###############################################################################
...
def update_hsegments(hsegments,hline_y_int,hline_y_frac,hline_xmin,hline_xmax):
    ...
    if hsegments_int_val is not None: 
        ...
        if hsegments_frac_val is not None:            
            insert_segment_main(hsegments_frac_val,hsegments_frac_val.head,
...
def update_vsegments(vsegments,vline_x_int,vline_x_frac,vline_ymin,vline_ymax):
    ...
    if vsegments_int_val is not None: 
        ...
        if vsegments_frac_val is not None:            
            insert_segment_main(vsegments_frac_val,vsegments_frac_val.head,
...
# to print the trees of horizontal/vertical segments - for debugging 
...
# insert a new line segment in a linked list (main function)
...
###############################################################################
# BLOCK : generate new subsegments after a line segment is dropped 
###############################################################################
...
def gen_list_int_main(segments,line_c_int,line_c_frac,line_min,line_max):
    ...
    if segments_int_val is not None: 
        ...
        if segments_frac_val is not None: 
            list_intervals,left_end_b,right_end_b=gen_list_int_core(
                                                      segments_frac_val.head,
                                                      line_min,line_max)     
        else:
            list_intervals=[line_min,line_max]
            left_end_b = True; right_end_b= True
    else:     
        list_intervals=[line_min,line_max]          
        left_end_b = True; right_end_b= True
    
    return list_intervals, left_end_b, right_end_b  
...
def gen_list_int_core(current_node,new_seg_l,new_seg_r):
    list_intervals=[]; left_end_b=False; right_end_b=False
    while True: 
        if new_seg_l<current_node.data[0]: 
            list_intervals.append(new_seg_l)
            list_intervals,right_end_b=gen_list_int_right(current_node, 
                                                          new_seg_l,
                                                          new_seg_r,
                                                          list_intervals)  
            left_end_b=True
...
# auxiliary function for generating a list of new subsegments 
    ...
    while True:
        if new_seg_r<current_node.data[0]: 
            ...
            if current_node.next is None: 
                ...
                current_node=current_node.next      
    return list_intervals, right_end_b         

###############################################################################
# BLOCK: generate knot vectors for new basis functions
###############################################################################
# This block is divided into two parts.
###############################################################################
# Part 1:
###############################################################################
# here we generate integer knots for new basis functions   
def knots_int(list_intervals,left_end_b,right_end_b,line_type,hcells,
              vcells,line_c_int,line_c_frac,div):
    list_knots_int=[] 
    left_end_int=list_intervals[0] 
    right_end_int=list_intervals[-1]  
    if line_type==0: 
        m=hcells; n=vcells  
    else: 
        m=vcells; n=hcells
    ...
    for i in range(0,L-2,2):
        while left_knot<=list_intervals[i+1]-1:
            tuple = tuple_knots_int(left_knot,line_type,n,line_c_int,
                                    line_c_frac,div)
...
# here we form a knot tuple for a basis function 
def tuple_knots_int(left_knot,line_type,n,line_c_int,line_c_frac,div): 
    ...
    if line_c_int==0: 
        ...
        tuple_two=(line_c_int-1,line_c_int,line_c_int+line_c_frac/div,
                   line_c_int+1,line_c_int+2)
    
    tuple = form_tuple(tuple_one,tuple_two,line_type)
...
# Part 2: 
###############################################################################
# here we generate knot vectors with one noninteger knot
def knots_nonint(segments,list_intervals,left_end_b,right_end_b,
                 line_c_int,line_c_frac,div,div2,line_type,hcells,vcells):
    list_knots_nonint=[] 
    left_end_int=list_intervals[0] 
    right_end_int=list_intervals[-1]-1  
    
    if left_end_b==True: 
        tuple_one_list=[left_end_int,"blank",left_end_int+1,left_end_int+2,
                        left_end_int+3]; blank_ind=2
                
        gen_knots(segments,left_end_int,line_c_int,line_c_frac,div,div2,
                  tuple_one_list,blank_ind,list_knots_nonint,line_type)
    else:        
        if (line_type==0) and (left_end_int<hcells-1): 
            tuple_one_list=[left_end_int-1,left_end_int,"blank",left_end_int+1,
                            left_end_int+2]; blank_ind=3
        if (line_type==0) and (left_end_int==hcells-1):
            tuple_one_list=[left_end_int-2,left_end_int-1,left_end_int,"blank",
                            left_end_int+1]; blank_ind=4
    ...
    for i in range(0,L-2,2):
        while mid_knot<=list_intervals[i+1]-1:
            tuple_one_list=[mid_knot-1,mid_knot,"blank",mid_knot+1,mid_knot+2] 
            blank_ind=3
                      
            gen_knots(segments,mid_knot,line_c_int,line_c_frac,div,div2,
                      tuple_one_list,blank_ind,list_knots_nonint,line_type)
            mid_knot=mid_knot+1  
    ...
    if right_end_b==True:  
        tuple_one_list=[right_end_int-2,right_end_int-1,right_end_int,"blank",
                        right_end_int+1] 
        blank_ind=4
        gen_knots(segments,right_end_int,line_c_int,line_c_frac,div,div2,
                  tuple_one_list,blank_ind,list_knots_nonint,line_type)     
    else: 
        tuple_one_list=[right_end_int-1,right_end_int,"blank",right_end_int+1,
                        right_end_int+2] 
        blank_ind=3
        gen_knots(segments,right_end_int,line_c_int,line_c_frac,div,div2,
                  tuple_one_list,blank_ind,list_knots_nonint,line_type)
...
def gen_knots(segments,index,line_c_int,line_c_frac,div,div2,tuple_one_list,
              blank_ind,list_knots_nonint,line_type):
    ...
    if segments_int_val is not None: 
        for key_frac_val in segments_int_val.keys(): 
            ...
            tuple_two = find_intersection(segments_frac_val.head,line_c_int,
                                          line_c_frac,div) 
...
def find_intersection(current_node,line_c_int,line_c_frac,div):    
    while current_node is not None:
        if line_c_int<current_node.data[0]:
            ...
            tuple=find_knots(current_node.data[0],current_node.data[1],
                             line_c_int,line_c_frac,div)
            return tuple
        else:   
            current_node=current_node.next
    return None
...
def find_knots(left_end,right_end,line_c_int,line_c_frac,div): 
    if line_c_int==left_end: 
        return (line_c_int,line_c_int+line_c_frac/div,line_c_int+1,
                line_c_int+2,line_c_int+3)
    elif line_c_int==right_end-1: 
        return (line_c_int-2,line_c_int-1,line_c_int,
                line_c_int+line_c_frac/div,line_c_int+1)    
    else: 
        return (line_c_int-1,line_c_int,line_c_int+line_c_frac/div,
                line_c_int+1,line_c_int+2)
       
###############################################################################
# end of Part 2
###############################################################################
# here we form a tuple
...
# here we read parameters from the input file
hcells, vcells, hdiv, vdiv = read_input_module.read_input() 

# here we ask how many lines to drop 
nlines = dialogue_module.ask_number_lines()
...
# in this array we store the knots of the B-spline basis functions
list_knots =[] 

# in these dictionaries we store horizontal/vertical segments
hsegments={}; vsegments={}

# in each iteration of the loop below a line from the Line list is added
for Lines_tuple in Lines:  
    
    # check if the line is horizontal or vertical 
    if Lines_tuple[0]==0: # the line is horizontal
        hline_y_int=Lines_tuple[1];hline_y_frac=Lines_tuple[2]
        hline_xmin=Lines_tuple[3];hline_xmax=Lines_tuple[4]

        # generate a list of new subintervals 
        list_intervals,left_end_b,right_end_b=gen_list_int_main(hsegments,
                                                                hline_y_int,
                                                                hline_y_frac,
                                                                hline_xmin,
                                                                hline_xmax)      
        ...
        if list_intervals: 
            # generate knots from the intersection with the original grid 
            list_knots_int=knots_int(list_intervals,left_end_b,right_end_b,0,
                                     hcells,vcells,hline_y_int,hline_y_frac,
                                     vdiv) 

            # generate knots from the intersection with the previous lines
            list_knots_nonint=knots_nonint(vsegments,list_intervals,
                                           left_end_b,right_end_b,
                                           hline_y_int,hline_y_frac,
                                           vdiv,hdiv,0,hcells,vcells) 

            # he we update the tree of horizontal line segments
            update_hsegments(hsegments,hline_y_int,hline_y_frac,
                             hline_xmin,hline_xmax)
    else: # the line is vertical 
        vline_x_int=Lines_tuple[1]; vline_x_frac=Lines_tuple[2]
        vline_ymin=Lines_tuple[3]; vline_ymax=Lines_tuple[4]

        # generate a list of new subintervals
        list_intervals,left_end_b,right_end_b=gen_list_int_main(vsegments,
                                                                vline_x_int,
                                                                vline_x_frac,
                                                                vline_ymin,
                                                                vline_ymax)      
        ...
        if list_intervals:
            # generate knots from the intersection with the original grid
            list_knots_int=knots_int(list_intervals,left_end_b,right_end_b,1,
                                     hcells,vcells,vline_x_int,vline_x_frac,
                                     hdiv) 
            # generate knots from the intersection with the previous lines
            list_knots_nonint=knots_nonint(hsegments,list_intervals,
                                           left_end_b,right_end_b,
                                           vline_x_int,vline_x_frac,
                                           hdiv,vdiv,1,hcells,vcells)
            
            # here we update the tree of vertical segments
            update_vsegments(vsegments,vline_x_int,vline_x_frac,
                             vline_ymin,vline_ymax)    

    # here we update list_knots
    list_knots = list_knots + list_knots_int + list_knots_nonint

# here we save a list of knots in a file    
record_knots_module.save_knots(list_knots)   

# here we ask if the user would like to see animation
dialogue_module.animation_yesno(len(list_knots))

# here we draw a spline
draw_spline_module.main(list_knots,hcells,vcells)  
